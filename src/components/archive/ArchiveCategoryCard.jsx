...
export default function ArchiveCategoryCard({ title, description, href, image, count, statusLabel }) {
  return (
    <Link to={href} className="clean-card clean-hover group block overflow-hidden rounded-xl">
      <div className="relative h-36 overflow-hidden border-b border-[#3e3e42]">
        ...
        {statusLabel ? (
          <div className="absolute bottom-3 left-3 rounded-md border border-[#3e3e42] bg-[#252526]/90 px-2.5 py-1 font-mono-soft text-[10px] uppercase tracking-[0.15em] text-[#4ec9b0]">
            {statusLabel}
          </div>
        ) : null}
      </div>
      <div className="p-5">
        ...
        <h3 className="mt-4 text-xl font-semibold tracking-[-0.025em] text-[#d4d4d4]">{title}</h3>
        <p className="mt-2 text-sm leading-6 text-[#b7b7b7]">{description}</p>
        <p className="mt-4 font-mono-soft text-xs text-[#4ec9b0] transition group-hover:translate-x-1">open() →</p>
      </div>
    </Link>
  )
}
