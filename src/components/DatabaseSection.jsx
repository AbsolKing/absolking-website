      ...
      <div className="mx-auto max-w-6xl overflow-hidden rounded-[1.75rem] glass-panel sm:rounded-[2rem]">
        <div className="grid gap-0 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="border-b border-white/10 p-5 sm:p-8 lg:border-b-0 lg:border-r lg:p-10">
            ...
            <h2 className="mt-3 max-w-lg font-serif text-[2.5rem] leading-tight tracking-tight text-slate-50 sm:mt-4 sm:text-5xl">
              A media archive with room to grow.
            </h2>
            <p className="mt-4 max-w-xl leading-8 text-slate-200/80 sm:mt-5 sm:leading-9">
            ...
            <Link
              to="/database"
              className="mt-6 inline-flex rounded-full border border-white/10 bg-white/[0.07] px-4 py-2 text-xs font-medium uppercase tracking-[0.22em] text-slate-50 transition hover:border-sky-200/30 hover:bg-white/[0.12] sm:mt-8"
            >
              Browse the Archive
            </Link>
          </div>
          ...
          <div className="grid gap-px bg-white/10 sm:grid-cols-2">
            {sections.map((item) => (
              <Link key={item.title} to={item.href} className="bg-[#0a1320]/65 p-5 backdrop-blur-xl transition hover:bg-[#0d192a]/80 sm:p-7">
                <p className="font-mono-soft text-[11px] uppercase tracking-[0.24em] text-slate-400">Collection</p>
                <p className="mt-3 text-xl font-semibold text-slate-50 sm:mt-4 sm:text-2xl">{item.title}</p>
                <p className="mt-2 text-sm leading-7 text-slate-300/75 sm:mt-3">{item.note}</p>
              </Link>
            ))}
          </div>
        </div>
